...
def calculate_sql_file(sql_text, file_path):
    ...
    for condition in conditions:
        ...
        if ">=" in condition or ">" in condition:
            ...
            score = 0
            if value[1] < file_json["df_min"][column_name] or value[0] > file_json["df_max"][column_name]:
                score = 0
            elif value[0] >= file_json["df_min"][column_name] and value[1] <= file_json["df_max"][column_name]:
                score = (
                        file_json["cnt"]
                        * (value[1] - value[0])
                        / (file_json["df_max"][column_name] - file_json["df_min"][column_name])
                ...
                score = (
                        file_json["cnt"]
                        * (file_json["df_max"][column_name] - value[0])
                        / (file_json["df_max"][column_name] - file_json["df_min"][column_name])
                )
            elif value[0] < file_json["df_min"][column_name] and value[1] <= file_json["df_max"][column_name]:
                score = (
                        file_json["cnt"]
                        * (value[1] - file_json["df_min"][column_name])
                        / (file_json["df_max"][column_name] - file_json["df_min"][column_name])
                )
            # score = (
            #         file_json["cnt"]
            ...
            # print(column_name)
            # print(value)
            # print(file_json["df_min"][column_name])
            # print(file_json["df_max"][column_name])
            value = value.split("'")[1]
            ...
            for value in (
                ...
                # print(value)
                if (value.split("'")[1] >= file_json["df_min"][column_name]) and (
                        value.split("'")[1] <= file_json["df_max"][column_name]
                ):
                    score += file_json["cnt"] / file_json["cntd"][column_name]
...
if __name__ == "__main__":
    ...
    # sql_text_ = """SELECT SUM(O_EXTENDEDPRICE * O_DISCOUNT) AS revenue FROM ssbflat WHERE O_ORDERDATE BETWEEN 19940101 AND 19940131 AND O_DISCOUNT BETWEEN 5 AND 6 AND O_QUANTITY BETWEEN 26 AND 30;"""
    sql_text_2 = """SELECT (O_ORDERDATE DIV 10000) AS YEAR,S_NATION, P_CATEGOR Y, SUM(LO_REVENUE - LO_SUPPLYCOST) AS profit FROM ssbflat WHERE S_REGION = 'AFRICA' AND O_ORDERDATE between 19920101 AND 19981231 AND P_MFGR IN ('MFGR#1', 'MFGR#2') GROUP BY YEAR, S_NATION, P_CATEGORY ORDER BY YEAR ASC, S_NATION ASC, P_CATEGORY ASC;"""
    # print(handle_sql(sql_text_))
    # calculate_sql_file(sql_text_, file_name)

    main_pipe([ sql_text_2], file_path_list)
